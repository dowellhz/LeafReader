...
API Key 保存在 macOS Keychain 中。普通启动、打开文档和选择文字不会读取 Keychain；应用只在你实际执行 AI 操作、打开相关设置或运行诊断时读取所需密钥。只有使用 AI 功能时，选中的文本、问题或用于分析的片段才会发送到你配置的模型服务。
...
- 使用“文件 > 备份用户数据...”备份词汇、学习状态、阅读笔记、笔记图片、会话和偏好设置。
- API Key、模型文件和可重建缓存不会写入备份。
- 使用“文件 > 恢复用户数据...”选择并验证备份。为避免替换已打开的数据库，恢复会安排在下次启动、Reader 窗口创建之前执行。
- 恢复不会覆盖当前 Mac 的 Keychain API Key。
