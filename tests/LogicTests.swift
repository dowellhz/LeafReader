...
private let tests: [(String, () throws -> Void)] = [
    ...
    ("Automatic embedding avoids credential access", AISettingsLogicTests.testAutomaticEmbeddingConfigurationAvoidsCredentialAccess),
    ("User data backup round trip", UserDataBackupServiceTests.testRoundTripExcludesCredentialsAndPreservesCurrentKeyReferences),
    ("User data backup tamper rejection", UserDataBackupServiceTests.testTamperedPayloadIsRejectedBeforeMutation),
    ("User data restore rollback", UserDataBackupServiceTests.testFailedRestoreRollsBackReplacements),
    ("User data interrupted restore recovery", UserDataBackupServiceTests.testInterruptedRestoreRecoveryUsesJournal),
    ("User data backup symlink rejection", UserDataBackupServiceTests.testBackupRejectsManagedAssetSymlinks),
