...
SUPERTONIC_RUNTIME="${SUPERTONIC_RUNTIME:-$HOME/.local/share/leafreader/supertonic-coreml/fluidaudiocli}"
...
if [[ -x "$SUPERTONIC_RUNTIME" ]]; then
  ...
  cp "$SUPERTONIC_RUNTIME" "$APP_PATH/Contents/Resources/SpeechRuntimes/supertonic-coreml/fluidaudiocli"
  chmod 755 "$APP_PATH/Contents/Resources/SpeechRuntimes/supertonic-coreml/fluidaudiocli"
  strip -x "$APP_PATH/Contents/Resources/SpeechRuntimes/supertonic-coreml/fluidaudiocli" || true
else
...
RUNTIME_EXECUTABLES=(
  ...
  "$APP_PATH/Contents/Resources/SpeechRuntimes/supertonic-coreml/fluidaudiocli"
)
